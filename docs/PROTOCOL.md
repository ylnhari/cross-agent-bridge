...
visible again. After observation, the live adapter owns recovery. If its
exclusive endpoint lease is released or expires and another adapter takes over,
claimed, injected, observed, or acted deliveries without a final reply are
...
using the same endpoint is rejected until the first lease is released or
expires. This prevents two host processes from injecting the same session
...
- Claude Channel writes `notifications/claude/channel`; Claude's explicit tool
  calls record observation, progress, sends, and replies. Channel events queue
  in order while Claude is busy.
...
not run a polling loop between turns.
